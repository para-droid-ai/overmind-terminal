...
const getSystemLogPrefixAndContext = (entry: SystemLogEntry, forClipboard: boolean = false): string => {
  const { type, source, turn: entryTurn, phase: entryPhase, message } = entry;
  let prefix = "";
  ...
  if (forClipboard) {
    prefix = `[Turn ${entryTurn}, ${phaseDisplay}] ${source ? source + ' ' : ''}(${type}): `;
  ...
  if ((type === 'AI_ACTION' && source) || type === 'EVENT') {
      context = ` <span class="opacity-60 text-[0.9em]">[T${entryTurn}, ${phaseDisplay}]</span> `;
  }
  
  return `<span class="${mainColorClass}">${prefix}${context}${message}</span>`;
};
...
const NoosphericSidebar: React.FC<NoosphericSidebarProps> = ({ 
  ...
  const [expandedSection, setExpandedSection] = useState<'systemLog' | 'battleHistory' | null>(null);
  ...
  const handleCopySystemLog = async () => {
    ...
    try {
      ...
      // Optionally, add a visual cue for error
    }
  };
  ...
  const renderExpandedLogView = (
    type: 'systemLog' | 'battleHistory',
    title: string,
    entries: SystemLogEntry[] | BattleLogEntry[]
  ) => {
    const renderEntryContent = (entry: any, index: number) => { // Added index for key
      if (type === 'systemLog') {
        return <li key={entry.id || `syslog-${index}`} className="leading-snug" dangerouslySetInnerHTML={{ __html: getSystemLogPrefixAndContext(entry as SystemLogEntry) }} />;
      } else if (type === 'battleHistory') {
        const battleEntry = entry as BattleLogEntry;
        return (
          <li key={battleEntry.id || `battlelog-${index}`} className="border-t border-dashed border-[var(--color-border-strong)] pt-1">
            <p>T{battleEntry.turn}: {mapNodes[battleEntry.nodeId]?.label || battleEntry.nodeId} ({mapNodes[battleEntry.nodeId]?.regionName || 'Unknown Region'})</p>
            <p><span className={battleEntry.attacker === 'GEM-Q' ? 'text-[var(--color-ai1-text)]' : 'text-[var(--color-ai2-text)]'}>{battleEntry.attacker}</span> vs <span className={battleEntry.defender === 'GEM-Q' ? 'text-[var(--color-ai1-text)]' : battleEntry.defender === 'AXIOM' ? 'text-[var(--color-ai2-text)]' : 'text-[var(--color-text-muted)]'}>{battleEntry.defender}</span></p>
            <p>Outcome: <span className={battleEntry.outcome.includes("ATTACKER") ? 'text-[var(--color-system-message)]' : 'text-[var(--color-info)]'}>{battleEntry.outcome.replace('_', ' ')}{battleEntry.nodeCaptured ? ' (Node Captured)' : ''}</span></p>
            <p>Losses A/D: {battleEntry.attackerLosses}/{battleEntry.defenderLosses}</p>
          </li>
        );
      }
      return null;
    ...
    return (
      <div className="p-2.5 flex flex-col bg-[var(--color-bg-terminal)] h-full rounded-md border-2 border-[var(--color-primary-500)] shadow-2xl">
        <div className="flex justify-between items-center mb-2 flex-shrink-0">
          <h4 className="text-md font-semibold text-[var(--color-text-heading)]">{title} (Expanded)</h4>
          <button
            onClick={() => setExpandedSection(null)}
            title="Close Expanded View"
            className="p-1 hover:bg-[var(--color-bg-button-secondary-hover)] rounded text-[var(--color-text-muted)]"
          >
        ...
        <ul className="text-xs space-y-0.5 overflow-y-auto flex-grow log-display pr-1 custom-scrollbar">
          {entries.length === 0 && <li className="italic text-[var(--color-text-muted)]">No entries.</li>}
          {entries.slice().reverse().map((entry, index) => renderEntryContent(entry, index))}
        </ul>
      </div>
    );
  };
  ...
  return (
    <aside className="w-full md:w-1/3 lg:w-[320px] flex flex-col gap-2 p-1 bg-[var(--color-bg-panel)] rounded-md shadow-lg overflow-hidden">
      {expandedSection === 'systemLog' ? (
        renderExpandedLogView('systemLog', 'System Log', systemLog)
      ) : expandedSection === 'battleHistory' ? (
        renderExpandedLogView('battleHistory', 'Battle History', battleLog)
      ) : (
        <>
          {/* Turn Info Panel */}
          ...
          <div className="flex-grow flex flex-col min-h-0 gap-2 overflow-hidden">
            <div className="p-2.5 flex flex-col bg-[var(--color-bg-terminal)] bg-opacity-50 flex-grow basis-3/5 min-h-0">
              <div className="flex justify-between items-center mb-1 flex-shrink-0">
                <h4 className="text-sm font-semibold text-[var(--color-text-heading)]">System Log</h4>
                <div className="flex items-center">
                    ...
                    <button onClick={() => setExpandedSection('systemLog')} title="Expand System Log" className="p-0.5 hover:bg-[var(--color-bg-button-secondary-hover)] rounded text-[var(--color-text-muted)]">
            ...
            <div className="p-2.5 flex flex-col bg-[var(--color-bg-terminal)] bg-opacity-50 rounded-b-md flex-grow basis-2/5 min-h-0">
              <div className="flex justify-between items-center mb-1 flex-shrink-0">
                <h4 className="text-sm font-semibold text-[var(--color-text-heading)]">Battle History (Last 5)</h4>
                 <div className="flex items-center">
                    <button onClick={() => setExpandedSection('battleHistory')} title="Expand Battle History" className="p-0.5 hover:bg-[var(--color-bg-button-secondary-hover)] rounded text-[var(--color-text-muted)] mr-1">
                         <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m4.5 4.5h-4.5m4.5 0v4.5m0-4.5L15 15" />
                         </svg>
                    </button>
